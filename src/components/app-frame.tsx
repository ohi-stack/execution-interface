      ...
      <header className="sticky top-0 z-30 border-b border-gold-300/15 bg-abyss/82 px-4 py-4 shadow-sovereign backdrop-blur-2xl">
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Link href="/status" className="rounded-full border border-purple-300/20 bg-purple-300/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-purple-100">
                ...
                {section.links.map((link) => (
                  <li key={link.href}>
                    {link.href.startsWith('http') ? (
                      <a href={link.href} target="_blank" rel="noreferrer" className="text-slate-300 hover:text-cyan-300">
                        {link.label}
                      </a>
                    ) : (
                      <Link href={link.href} className="text-slate-300 hover:text-cyan-300">
                        {link.label}
                      </Link>
                    )}
                  </li>
